...
class CustomCLIP_teacher(nn.Module):
    ...
    def forward(self, image=None, label=None, sp=False):
        ...
        text_features = text_features + topk  # add topk to text_features
        if sp is False:
...
class CustomSPLECLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        if self.prompt_learner.training:
            ...
            #print("image_features_s.shape before:",  type(image_features_s))
            #[27, 512]
            #print(image_features_s)
            #image_features_s=torch.tensor(image_features_s,dtype=torch.float32)
            #print(image_features_s[0].shape)
            # image_features_moe,logit_router=self.moe(image_features_sp)  
   
            # image_features_moe=F.normalize(image_features_moe, dim=-1)
        #    # print(image_features_moe.shape)   #[27, 768]
            #image_features_moe=image_features_moe[:,0,:]
           ...
           # image_features_s=0.001*image_features_moe+image_features_s
            ...
            # image_features_sp=0.01*image_features_s+image_features_sp
