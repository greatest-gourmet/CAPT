...
def load_backbone_prompt_vector(cfg):
    ...
    kd_prompt_learner = torch.load(model_path, map_location="cuda", weights_only=False)["state_dict"]
    kd_vis_prompts_list = []
    ...
    src_prompt_learner = torch.load(src_model_path, map_location="cuda",weights_only=False)["state_dict"] 
...
class MoE(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 在这里更新 self.text (仅作记录，不影响 attention)
        ...
        # gate score (只取最大 expert 的得分)
        gate_score = routing_weights.max(dim=-1).values.view(batch_size, sequence_length, 1)
        out = x * gate_score

        probs = F.softmax(router_logits.detach(), dim=-1)
        load = probs.mean(dim=0)  # 每个 expert 的平均使用率
        self.loss_moe = (load * torch.log(load + 1e-9)).sum()  # 负熵，越平均越小
        self.loss_moe = -self.loss_moe  # 转为正的损失值，越均衡越低


        return out , router_logits, self.loss_moe
...
class AdapterLearner(nn.Module):
    def __init__(self, cfg,  clip_model):
        ...
        self.text_adapter = self._build_adapter(
           ...
           # cfg.TRAINER.LWEIB.ADAPTER_START,        #这边要加配置
            self.start,
            #cfg.TRAINER.LWEIB.ADAPTER_END,           #这边要加配置
            self.end,
...
class CustomCLIP_teacher(nn.Module):
    ...
    def forward(self, image=None, label=None, sp=False):
        ...
        text_features = text_features + 0.1*topk  # add topk to text_features
        if sp is False:
        ...
        #image_features=self.alpha*image_features+(1-self.alpha)*image_features_p
...
class CustomSPLECLIP(nn.Module):
    ...
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.adapter=AdapterLearner(cfg, clip_model)
        self.alpha=0.5
        self.moe=MoE( clip_model.visual.ln_post.weight.shape[0],3)   #
        #self.neg_sampling_ratio =7
        ...
        self.topk = self.prompt_learner.topk
        #self.VPT_topk_t = nn.Linear(self.topk.output_dim, clip_model.visual.output_dim).half()
    ...
    def forward(self, image, label=None):
        logit_scale = self.logit_scale.exp()
        #topk_t = self.VPT_topk_t(self.topk)     #[5, 768]
        text_adapter_parser, visual_adapter_parser = self.adapter()
        #alpha1=0
        ...
        if self.prompt_learner.training:
            ...
            if torch.rand(1).item()>self.alpha:     #0.5
                 self.alpha=random.uniform(1,2)*self.alpha 
                #  print("self.ahpha_stu",self.alpha)    #0.5~1
            else:
                 self.alpha= self.alpha
                #  print("self.ahpha",self.alpha)
 
            image_features_s=self.encode_image( [image_features_sp, visual_adapter_parser])
 
            image_features_s = image_features_s[0].detach().clone().requires_grad_(True).to(torch.float32)
            #print(image_features_s[0].shape)
            image_features_moe,logit_router,m_loss=self.moe(image_features_sp)  
       
            image_features_moe=F.normalize(image_features_moe, dim=-1)
 
           # image_features_s = F.normalize(image_features_s[0], dim=-1)
           # image_features_s=0.001*image_features_moe+image_features_s
            # print("image_features_moe.shape:", image_features_moe.shape)   #[27, 768]
            # print("image_features_s.shape:", image_features_s.shape)   #[27, 768]
           # image_features_s=image_features_s.float()
            image_features_sp=(1-self.alpha)*image_features_s+self.alpha*image_features_sp  
            # 1. 扩展后相加
            combined = 0.001*image_features_moe + image_features_sp.unsqueeze(1).expand(-1, image_features_moe.size(1), -1)
            # combined.shape == [16, 16, 768]

            # 2. 恢复到原来的形状（与 image_features_sp 一样）
            # 这里通常取平均或池化（因为你加了序列维度）
            image_features_sp= combined.mean(dim=1)   # [16, 768]

            #ima=self.moe(image_features_sp)
            tokenized_prompts = self.tokenized_prompts
            prompts = self.prompt_learner()  # ViT-L/14: [n_cls, 77, 768]
            # print("*****prompts.shape:", prompts.shape)     #[50, 77, 768]
            # print("$$$$$tokenized_prompts.shape:", tokenized_prompts.shape)    #[50, 77]
            ...
            #print("$$$$$$text_features_sp.shape:", text_features_sp.shape)   #[bs*TopK, 768]
            ...
            topk=self.topk.transpose(0,1)
            topk=proj(topk).transpose(0,1)
            text_features_sp = text_features_sp + 0.1*topk 
            #print("topk.shape:", topk.shape)   #[bs*TopK, 768]
           ...
           #print(f'label_ids size is {label_ids.size()}')
            ...
            for sample_id in range(0, input.size(0)):
                ...
                with open(pic_lib) as f:
                    ...
                    for obj_id in hn_labels:
                        # print("hn_obj_name:", hn_obj_name)
                        # print("available keys:", list(pics_for_selection["train"][0].keys())[:20])

                        hn_obj_name = class_label[obj_id]  # Get classname
                        pic_list = pics_for_selection["train"][0].get(hn_obj_name) 
                         # Search classname to get image list
                        # print("batch:", batch)
                        # print("pic_list:", pic_list, type(pic_list))
