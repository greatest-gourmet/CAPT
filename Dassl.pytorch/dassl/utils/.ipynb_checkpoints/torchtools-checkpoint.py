    ...
    for k, v in state_dict.items():
        if k in model_state:
            if v.shape == model_state[k].shape:
                ...
                print(f"⚠️ Skip or adapt param {k}: "
                      f"checkpoint {tuple(v.shape)} vs model {tuple(model_state[k].shape)}")
                # 如果是 prompt_learner 的 prefix/suffix，可以裁剪
                if "prompt_learner.token_prefix" in k or "prompt_learner.token_suffix" in k:
                    min_len = min(v.shape[0], model_state[k].shape[0])
                    new_state_dict[k] = v[:min_len]
        else:
            print(f"⚠️ Drop unexpected param: {k}")
