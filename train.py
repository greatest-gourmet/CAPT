...
import trainers.sple_promptkd
import trainers.promptkd_infer   

# import trainers.coop_stats
# import trainers.promptsrc_stats
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "1" # 指定使用编号为0的GPU     
...
def extend_cfg(cfg):
    ...
    cfg.SPLE.INFER_TOPK = 5  # Top-K for dynamic hard negative sampler
    cfg.SPLE.INFER_NONREPEAT = True  # No repeat object in a mini-batch
    cfg.SPLE.PIC_LIB = "../DATA/SPLE_database/SPLE_Caltech101.json"
    cfg.SPLE.LOSS_KD_WEIGHT = 0.5   
    cfg.SPLE.KD_INFER = ""   

    cfg.SPLE.SPLE_TRAINER = CN()
    cfg.SPLE.SPLE_TRAINER.SPLE_INIT = True   
    cfg.SPLE.SPLE_TRAINER.SPLE_BASE_TRAIN = True   

    cfg.SPLE.STACK = CN()
    cfg.SPLE.STACK.MODE = "void"  # [DISCARD] DO NOT CHANGE
    cfg.SPLE.STACK.WEIGHT = 0.5   
    cfg.SPLE.STACK.WEIGHT_FOR_NEW = 0.2   
